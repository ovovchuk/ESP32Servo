...
// The ChannelUsed array elements are 0 if never used, 1 if in use, and -1 if
// used and disposed (i.e., available for reuse)
/**
 * allocateTimer
    ...
    if (pwmChannel < 0) {
        for (int i = 0; i < 4; i++) {
            ...
            if (freqAllocated && timerCount[i] < 4) {
                ...
                // Serial.println("Free channel timer "+String(i)+" at freq
                // "+String(freq)+" remaining "+String(4-timerCount[i]));
                ...
                // Serial.println("Timer out of channels target="+String(freq)+" on timer
                // "+String(i)+" was
                //"+String(timerCount[i]));
            }
        }
...
double ESP32PWM::setup(int channel, float freq, uint8_t resolution_bits) {
    resolutionBits = resolution_bits;
    pwmChannel = channel;
    if (attached()) {
#ifdef ESP_ARDUINO_VERSION_MAJOR
...
void ESP32PWM::attachPin(uint8_t pin, uint8_t channel, double freq, uint8_t resolution_bits) {
    setup(channel, freq, resolution_bits);
    attachPin(pin);
}
    ...
    for (int i = 0; i < timerCount[getTimer()]; i++) {
        ...
        if (ChannelUsed[pwm] != NULL)
            if (ChannelUsed[pwm]->getTimer() == getTimer()) {
                ...
                if (abs(diff) > 0.1) {
                    ESP_LOGW(TAG, "\tWARNING PWM channel %d	\

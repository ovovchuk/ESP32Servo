...
class ESP32PWM {
  private:
    ...
    double setup(int channel, float freq, uint8_t resolution_bits);
  ...
  public:
    ...
    void attachPin(uint8_t pin, uint8_t channel, double freq, uint8_t resolution_bits);
    bool attached() {
    ...
    /**
     * allocateTimer
     * @param a timer number 0-3 indicating which timer to allocate in this
     * library Switch to explicate allocation mode
     *
     */

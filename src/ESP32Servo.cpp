...
Servo::Servo() { // initialize this channel with plausible values, except pin #
                 // (we set pin # when attached)
    REFRESH_CPS = 50;
...
int Servo::attach(int pin, int min, int max) {
    ...
    if (pwm.hasPwm(pin)) {
        ...
        if (this->pinNumber < 0) // we are attaching to a new or previously detached
                                 // pin; we need to initialize/reinitialize
        {
            this->ticks = DEFAULT_PULSE_WIDTH_TICKS;
            this->timer_width = DEFAULT_TIMER_WIDTH;
            this->timer_width_ticks = pow(2, this->timer_width);
...
if(
        ...
        ESP_LOGE(TAG,
                 "This pin can not be a servo: %d Servo available on: "
                 "1-21,35-45,47-48",
                 pin);
...
int Servo::attach(int pin, int channel, int min, int max) {
    ESP_LOGW(TAG, "Attempting to Attach servo on pin=%d min=%d max=%d", pin, min, max);
...
if(
    ...
    pwm.attachPin(this->pinNumber, channel, REFRESH_CPS, this->timer_width);
    ESP_LOGW(TAG, "Success to Attach servo : %d on PWM %d", pin, pwm.getChannel());

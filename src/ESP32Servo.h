...
/*
 ...
 int read() - Gets the last written servo pulse width as an angle between 0 and
 180. int readMicroseconds()   - Gets the last written servo pulse width in
 microseconds. bool attached() - Returns true if this servo instance is
 attached. void detach() - Stops an the attached servo, frees its attached pin,
 and frees its channel for reuse).
...
class Servo {
  ...
  public:
    ...
    int attach(int pin); // attach the given pin to the next free channel, returns
                         // channel number or 0 if failure
    int attach(int pin, int min,
               int max); // as above but also sets min and max values for writes.
    int attach(int pin, int channel, int min, int max);
    void detach();
    void write(int value);             // if value is < MIN_PULSE_WIDTH its treated as an
    ...
    int readMicroseconds(); // returns current pulse width in microseconds for
                            // this servo
    int readTicks();        // returns current ticks, the smallest increment the servo
                            // can handle
  ...
  private:
    ...
    //   static int ServoCount;                             // the total number of
    //   attached servos static int ChannelUsed[];                          //
    //   used to track whether a channel is in service int servoChannel = 0; //
    ...
    int timer_width_ticks =
        DEFAULT_TIMER_WIDTH_TICKS; // no. of ticks at rollover; varies with width
